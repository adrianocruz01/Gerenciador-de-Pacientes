...
@Injectable()
export class SearchIntraOperativeService {
    constructor(private readonly prisma: PrismaService) {}

    async execute(paciente_procedimento_id: string) {
        const paciente_ProcedimentoExists = await this.prisma.ficha_Intraoperatoria.findFirst({
        ...
        const form = await this.prisma.ficha_Intraoperatoria.findFirst({
            where: {
