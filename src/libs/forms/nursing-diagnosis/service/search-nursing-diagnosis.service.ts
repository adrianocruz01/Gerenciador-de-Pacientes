...
@Injectable()
export class SearchNursingDiagnosisService {
    constructor(private readonly prisma: PrismaService) {}

    async execute(paciente_procedimento_id: string) {
        const paciente_ProcedimentoExists = await this.prisma.ficha_Diagnostico_Enfermagem.findFirst({
        ...
        const form = await this.prisma.ficha_Diagnostico_Enfermagem.findFirst({
            where: {
