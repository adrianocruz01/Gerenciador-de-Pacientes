...
@Injectable()
export class SearchMaterialControlService {
    constructor(private readonly prisma: PrismaService) {}

    async execute(paciente_procedimento_id: string) {
        const paciente_ProcedimentoExists = await this.prisma.ficha_Controle_Material.findFirst({
        ...
        const form = await this.prisma.ficha_Controle_Material.findFirst({
            where: {
