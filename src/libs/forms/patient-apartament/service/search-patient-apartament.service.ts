...
@Injectable()
export class SearchPatientApartamentService {
    constructor(private readonly prisma: PrismaService) {}

    async execute(paciente_procedimento_id: string) {
        const paciente_ProcedimentoExists = await this.prisma.ficha_Encaminhamento_Paciente.findFirst({
        ...
        const form = await this.prisma.ficha_Encaminhamento_Paciente.findFirst({
            where: {
