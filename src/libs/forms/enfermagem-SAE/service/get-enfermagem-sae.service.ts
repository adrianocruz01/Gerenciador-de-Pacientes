...
@Injectable()
export class GetEnfermagemSaeFormService {
    constructor(private readonly prisma: PrismaService) {}

    async execute(paciente_procedimento_id: string) {
        const paciente_ProcedimentoExists = await this.prisma.paciente_Procedimento.findFirst({
        ...
        const form = await this.prisma.ficha_SAE_Triagem.findFirst({
            where: {
